    ...
    <div class="flex ">
        <div class="w-full ">
            ...
            <section class="flex flex-col break-words bg-black sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg ">
                <header class="font-semibold bg-black text-white py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md border-b-2 border-yellow-300 ">
                    {{ __('Reset Password') }}
                </header>
                ...
                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8" method="POST" action="{{ route('password.email') }}">
                    ...
                    <div class="flex flex-wrap justify-center items-center space-y-6 pb-6 sm:pb-10 sm:space-y-0 sm:justify-between">
                        <button type="submit"
                        class="w-full select-none font-bold whitespace-no-wrap p-3 rounded-lg text-base leading-normal no-underline text-black bg-yellow-300 hover:bg-yellow-200 sm:w-auto sm:px-4 sm:order-1">
                            {{ __('Send Password Reset Link') }}
                        </button>

                        <p class="mt-4 text-xs text-yellow hover:text-blue-700 whitespace-no-wrap no-underline sm:text-sm sm:order-0 sm:m-0">
                            <a class="text-yellow-300 hover:text-yellow-200" href="{{ route('login') }}">
                                {{ __('Back to login') }}
                            </a>
                        </p>
                    </div>
